...
export const AboutSection: React.FC<AboutSectionProps> = ({ aboutItems }) => {
  ...
  return (
    <section id="nosotros" className="py-24 bg-slate-100/70 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <span className="text-xs font-bold uppercase tracking-widest text-blue-700 bg-blue-100 px-3 py-1 rounded-full border border-blue-200">
            {t.about.badge}
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {aboutItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`text-left p-4 rounded-xl transition-all flex items-center justify-between border ${
                    isActive
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-lg ${
                        isActive ? 'bg-blue-800 text-amber-300' : 'bg-slate-100 text-blue-700'
                      }`}
                    >
                      {getIcon(item.icon)}
                    </div>
                  ...
                  <ChevronRight
                    className={`w-4 h-4 shrink-0 transition-transform ${
                      isActive ? 'text-amber-300 translate-x-1' : 'text-slate-400'
                    }`}
                  />
                </button>
              );
          ...
          <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-8 sm:p-10 shadow-sm relative overflow-hidden">
            {currentItem && (
              <div className="space-y-6">
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs text-slate-500 bg-slate-50/70 -mx-8 -mb-8 p-6 rounded-b-2xl">
